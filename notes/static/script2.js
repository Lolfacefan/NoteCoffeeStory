...
render('/api/tasks/', taskRespawn);
render('/api/tasks/done/0/10', tasksDoneRespawn);
...
window.onscroll = function (ev) {
    if ((window.innerHeight + window.pageYOffset + 1) >= document.body.offsetHeight) {
        const req = new Promise((event) => {
            if (permitRender){
                renderFooter(`/api/tasks/done/${numerIteration*10}/${numerIteration*10+10}`, tasksDoneRespawn);
